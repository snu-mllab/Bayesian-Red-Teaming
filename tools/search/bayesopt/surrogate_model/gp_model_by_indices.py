...
class MyGPModelByInd(BaseModel):
    def __init__(self, emb : torch.Tensor = None, fit_iter : int = 20):
        self.model = None 
    ...
    def fit(self, indices, scores):
        train_X = self.emb[torch.LongTensor(indices)].cuda()
    ...
    def acquisition(self, indices, bias=None, add_mean=0.0):
        eval_X = self.emb[indices].to(dtype=torch.double).cuda()
        with torch.no_grad():
